...
const Send: NextPage = () => {
  ...
  useEffect(() => {
    const getTokenAccountBalance = async () => {
      ...
      const mintData = await connection.getTokenSupply(mint_pk);
    ...
    <>
      ...
      {!finished && (
        ...
        >
            ...
            rules={[
              ...
              },
              {
                async validator(_, value) {
                  const pdaInfo = await connection.getAccountInfo(new PublicKey(value))
                  if (pdaInfo) {
          ...
          <span style={{opacity: '60%', color: 'white', marginTop: '2px', marginLeft: '230px'}}>balance: {tokenBalance / Math.pow(10, decimals)}</span>
